...
export default function Index() {
        ...
        async function boot() {
            ...
            async function tryLoad() {
                if (nbParam) {
                    ...
                    setCells(Array.isArray(found.cells) ? found.cells : [{ id: crypto.randomUUID(), language: "python", code: defaultSnippets.python }]);
                    return true;
                }
                ...
                if (list.length > 0) {
                    ...
                    setCells(Array.isArray(latest.cells) ? latest.cells : [{ id: crypto.randomUUID(), language: "python", code: defaultSnippets.python }]);
                    setParams((p) => {
                ...
                const created = await api("/api/notebooks", { method: "POST", body: JSON.stringify({}) });
                setNb(created);
                setTitle(created.title || "Untitled Notebook");
                setCells(Array.isArray(created.cells) ? created.cells : [{ id: crypto.randomUUID(), language: "python", code: defaultSnippets.python }]);
                setParams((p) => {
                    const np = new URLSearchParams(p);
    ...
    const addCell = (lang) => setCells((c) => [...c, { id: crypto.randomUUID(), language: lang, code: defaultSnippets[lang] }]);
    const updateCell = (id, patch) => setCells((list) => list.map((c) => (c.id === id ? { ...c, ...patch } : c)));
            ...
            <div>
                ...
                <main>
                    <div className="welcome">
                        ...
                        <p>
                            Colaboratory, or "Colab" for short, lets you write and execute code through your browser. Perfect for data analysis, machine learning and education.
                        </p>
                        ...
                        <div className="welcome-features">
                            ...
                            <div className="welcome-feature">
                                ...
                                <div className="welcome-feature-desc">Run Python, C++ and JS with safe, sandboxed execution.</div>
                            </div>
                        </div>
                        <div className="welcome-section">
                            <div className="welcome-section-title">Perfect for data science and machine learning</div>
                ...
                <main className="container" style={{ maxWidth: 1000, margin: '0 auto', padding: '2rem 0' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            />
                            <span style={{ fontSize: 14, background: '#fef3c7', color: '#b45309', borderRadius: 6, padding: '2px 8px' }}>Auto-saved</span>
                        </div>
                        <div style={{ display: 'flex', gap: 8 }}>
                            <button onClick={() => addCell('python')} className="button button-secondary">+ Python</button>
                            <button onClick={() => addCell('cpp')} className="button button-secondary">+ C++</button>
                            <button onClick={() => addCell('javascript')} className="button button-secondary">+ JavaScript</button>
                            <button onClick={runAll} className="button button-primary">Run all</button>
                        </div>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 32, paddingBottom: 64 }}>
                        {cells.map((cell) => (
                            <div key={cell.id} style={{ border: '1px solid #e5e7eb', borderRadius: 12, background: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #f3f4f6', background: '#f8fafc', padding: '10px 16px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        ...
                                        <button onClick={() => runCell(cell)} className="button button-primary" style={{ fontSize: 15, padding: '6px 18px' }}>
                                            {cell.running ? 'Running...' : 'Run'}
                                        </button>
                                    </div>
                                    <button onClick={() => removeCell(cell.id)} className="button button-secondary" style={{ fontSize: 15, padding: '6px 18px' }}>Delete</button>
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'row', gap: 0 }}>
                                    <textarea
                                        ...
                                        spellCheck={false}
                                        className="form-input"
                                        style={{ minHeight: 120, width: '100%', resize: 'vertical', fontFamily: 'Fira Mono, Consolas, monospace', fontSize: 15, border: 'none', borderRadius: 0, borderRight: '1px solid #f3f4f6', background: '#f9fafb', color: '#222', padding: 16 }}
                                    />
                                    <div style={{ flex: 1, padding: 16, background: '#f9fafb', borderTopRightRadius: 12, borderBottomRightRadius: 12 }}>
                                        <div style={{ fontSize: 13, fontWeight: 500, color: '#888' }}>Output</div>
                                        <pre style={{ marginTop: 8, whiteSpace: 'pre-wrap', fontSize: 15, color: '#059669' }}>{cell.stdout}</pre>
                                        {cell.stderr && (
                                            <pre style={{ marginTop: 8, whiteSpace: 'pre-wrap', fontSize: 15, color: '#dc2626' }}>{cell.stderr}</pre>
                                        )}
                                    </div>
                                </div>
                            </div>
